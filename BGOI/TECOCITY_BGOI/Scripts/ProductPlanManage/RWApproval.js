            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success == "true") {
                        ...
                        ShowIframe1("审批", "../ProduceManage/RWSP?id=" + texts, 500, 400, '');
                    }
                    else {
                        return;
                    }
...
function jq() {
    ...
    jQuery("#list").jqGrid({
        ...
        colNames: ['', '任务单号', '订货单位', '创建人', '创建时间', '状态', 'State', '审批编号'],
        colModel: [
        { name: 'IDCheck', index: 'Id', width: 20 },
        { name: 'RWID', index: 'RWID', width: 150 },
        { name: 'OrderUnit', index: 'OrderUnit', width: 100 },
        { name: 'CreateUser', index: 'CreateUser', width: $("#bor").width() - 800 },
...
function select(rowid) {
    ...
    RelenvceID = jQuery("#list").jqGrid('getRowData', rowid).RWID;
    reload1();
    reload2(PID);
}
...
function reload2(PID) {
    $("#list2").jqGrid('setGridParam', {
        url: 'CreateProjectLogGrid',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, PID: PID, Type: Type },
...
function jq2(PID) {
    jQuery("#list2").jqGrid({
        ...
        colModel: [
        { name: 'IDCheck', index: 'Id', width: 20, hidden: true },
        { name: 'YYCode', index: 'YYCode', width: 120 },
        { name: 'Content', index: 'Content', width: 200 },
        { name: 'YYType', index: 'YYType', width: $("#bor").width() - 700 },
