...
$(document).ready(function () {
    ...
    $("#btnSave").click(function () {
        ...
        for (var i = 0; i < tbody.rows.length; i++) {
            ...
            var pId = tbody.getElementsByTagName("tr")[i].cells[1].innerText;//document.getElementById("MaterialNO" + i).innerHTML;
            ...
            var shId = tbody.getElementsByTagName("tr")[i].cells[10].innerText;//document.getElementById("SHID" + i).innerHTML;
            var orderId = tbody.getElementsByTagName("tr")[i].cells[11].innerText;//document.getElementById("OrderID" + i).innerHTML;
        ...
        $.ajax({
            url: "SaveProcureStockIn",
            type: "Post",
            data: {
                ListInID: ListInID, SubjectID: SubjectID, BatchID: BatchID, StockInTime: StockInTime, HouseID: HouseID, ProPlace: ProPlace, SHID: SHID,
                LoginName: LoginName, PID: PID, ProName: ProName, SpecsModels: SpecsModels, MainContent: MainContent, OrderID: OrderID, DrawID: DrawID, Remark1: Remark1,
...
function addProDetail(SHID, RKID) {
    $.ajax({
        url: "GetProcureDetail",
        type: "post",
        data: { SHID: SHID, RKID: RKID },
        dataType: "json",
        success: function (data) {
            ...
            if (json.length > 0) {
                for (var i = 0; i < json.length; i++) {
                    ...
                    html += '<td ><lable class="labMaterialNO' + rowCount + ' " id="MaterialNO' + rowCount + '">' + json[i].MaterialNO + '</lable> </td>';
                    ...
                    //html += '<td ><lable class="labUnitPrice' + rowCount + ' " id="UnitPrice' + rowCount + '">' + json[i].UnitPriceNoTax + '</lable> </td>';
                    html += '<td ><input type="text" id="UnitPrice' + rowCount + '" value="' + json[i].Price2 + '" style="width:30px;" onblur="OnBlurUnitPrice(this);"/></td>';


                    html += '<td ><input type="text" id="Total' + rowCount + '" value="' + json[i].TotalNoTax + '" style="width:60px;"/></td>';
                    ...
                    html += '<td style="display:none;"><lable class="labSHID' + rowCount + ' " id="SHID' + rowCount + '">' + json[i].SHID + '</lable> </td>';
                    html += '<td style="display:none;"><lable class="labOrderID' + rowCount + ' " id="OrderID' + rowCount + '">' + json[i].RKID + '</lable> </td>';
                    html += '<td style="display:none;"><lable class="labAmount' + rowCount + ' " id="Count' + rowCount + '">' + json[i].Amount + '</lable></td>';
                    html += '</tr>'
...
function AddNewProcure() {
    ShowIframe1("选择采购货品单", "../InventoryManage/ChangeProcure", 760, 350);
}
function GetAmount() {
    $("#AmountM").val("");
    var Amount = 0;
...
function DelRow() {
    ...
    if (one == false)
        ...
        var rowIndex = -1;
        var typeNames = ["RowNumber", "PID", "OrderContent", "Specifications", "Unit", "Amount", "UnitPrice", "Total", "Supplier", "Remark", "SHID", 'OrderID', 'Count'];
                ...
                for (var i = rowIndex; i < $("#" + tbodyID + " tr").length; i++) {
                    ...
                    for (var j = 0; j < tr.childNodes.length; j++) {
                        ...
                        for (var k = 0; k < typeNames.length; k++) {
                            ...
                            var olSHID = typeNames[k] + (parseInt(i) + 1);
                            var newid = typeNames[k] + i;

                            var reg = new RegExp(olSHID, "g");
