...
$(document).ready(function () {
    ...
    $("#btnPrint").click(function () {
        ...
        if (rowid == null) {
            ...
            var url = "PrintProtoStockIn?Info=" + escape(texts);
...
function AddProtoStockIn() {
    window.parent.parent.OpenDialog("新增撤样机入库", "../InventoryManage/AddProtoStockIn", 800, 450);
}
function jq() {
    ...
    jQuery("#list").jqGrid({
        url: 'ProtoStockInList',
        datatype: 'json',
        postData: {
            curpage: curPage, rownum: OnePageCount, BatchID: BatchID,
            ListInID: ListInID, ProType: ProType, Begin: Begin, End: End, State: State,
            Spec: Spec, IsHouseIDone: IsHouseIDone, IsHouseIDtwo: IsHouseIDtwo
        },
        loadonce: false,
        mtype: 'POST',
        jsonReader: {
        ...
        colNames: ['','序号', '入库单编号', '申请人', '申请日期', '入库时间', '入库人', '总金额', '产品库类型', '所属仓库', '备注', '状态'],
        colModel: [
        ...
        { name: 'ApplyPer', index: 'ApplyPer', width: 120, align: "center" },
        { name: 'ApplyTime', index: 'ApplyTime', width: 150, align: "center" },
...
function reload() {
    ...
    jQuery("#list").jqGrid({
        url: 'ProtoStockInList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, ApplyPer: ApplyPer, ListInID: ListInID, ProType: ProType, Begin: Begin, End: End, State: State, Spec: Spec, IsHouseIDone: IsHouseIDone, IsHouseIDtwo: IsHouseIDtwo },
...
function getSearch() {
    ...
    jQuery("#list").jqGrid('setGridParam', {
        ...
        datatype: 'json',
        postData: {
            curpage: curPage, rownum: OnePageCount, ApplyPer: ApplyPer,
            ListInID: ListInID, ProType: ProType, Begin: Begin,
            End: End, State: State, Spec: Spec, IsHouseIDone: IsHouseIDone, IsHouseIDtwo: IsHouseIDtwo
        },
        loadonce: false
...
function jq1(ListInID) {
    ...
    jQuery("#list1").jqGrid({
        ...
        colNames: ['序号', '产品编号', '产品名称', '规格型号', '单位', '数量', '单价', '厂家', '备注', '撤样机编号'],
        colModel: [
        { name: 'RowNumber', index: 'RowNumber', width: 50, align: "center" },
...
function reload1(ListInID) {
    ...
    $("#list1").jqGrid('setGridParam', {
        url: 'StockInDetialList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount1, ListInID: ListInID },
