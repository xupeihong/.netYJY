...
$(document).ready(function () {
    ...
    $("#btnSave").click(function () {
        ...
        $.ajax({
            ...
            type: "Post",
            data: {
                ListOutID: ListOutID, SubjectID: SubjectID, ProOutTime: ProOutTime, HouseID: HouseID, Use: Use, Count: Count, Purchase: Purchase,
                ProOutUser: ProOutUser, PID: PID, ProName: ProName, SpecsModels: SpecsModels, MainContent: MainContent, Remark1: Remark1,
            ...
            success: function (data) {
                ...
                if (json.length > 0) {
                    for (var i = 0; i < json.length; i++) {
                        ...
                        html += '<td ><input type="text" id="Amount' + rowCount + '" style="width:30px;"/></td>';// onblur="OnBlurAmount(this);"
                        ...
                        html += '<td ><lable class="labManufacturer' + rowCount + ' " id="COMNameC' + rowCount + '">' + json[i].COMNameC + '</lable> </td>';
                        html += '<td ><lable class="labRemark' + rowCount + ' " id="Remark' + rowCount + '">' + json[i].Remark + '</lable> </td>';
                     //   html += '<td ><lable class="labx' + rowCount + ' " id="x' + rowCount + '">' + json[i].x + '</lable> </td>';
