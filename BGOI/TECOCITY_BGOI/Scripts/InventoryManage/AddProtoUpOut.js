       ...
       // var UnitName = "";
        ...
        $.ajax({
            ...
            data: {
                ...
                StockOutCount: StockOutCount, UnitPrice: UnitPrice, TotalAmount: TotalAmount, Manufacturer: Manufacturer, Remark: Remark, Amount: Amount,
                StockOutCountActual: StockOutCountActual
            },
            async: false,
            ...
            success: function (data) {
                ...
                if (json.length > 0) {
                    for (var i = 0; i < json.length; i++) {
                        ...
                        html += '<td style="display:none;"><input type="text" id="Total' + rowCount + '" value="' + json[i].Total + '" style="width:60px;" /></td>';
                        //html += '<td ><lable class="labTotal' + rowCount + ' " id="Total' + rowCount + '">' + json[i].Total + '</lable></td>';
                        html += '<td style="display:none;"><lable class="labSupplier' + rowCount + ' " id="Supplier' + rowCount + '">' + json[i].Supplier + '</lable> </td>';
...
function AddNewOut() {
    ShowIframe1("选择上样机货品", "../InventoryManage/ChangeProtoUp", 500, 350);
}
...
function OnBlurAmount(rowcount) //求金额和
{
   ...
   // var strRow = newCount.charAt(newCount.length - 1);
    var strRow = newCount.substring(19);//StockOutCountActual
    var AmountC = $("#Count" + strRow).text();
    if (parseInt(AmountC) < parseInt(Count)) {
        alert("所填的数量已超过此上样机物品数量，请重新填写...");
        $("#" + newCount).val("0");
        $("#Total" + strRow).val("0");
        return;
    }
