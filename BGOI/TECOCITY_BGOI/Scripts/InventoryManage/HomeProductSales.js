   ...
   // window.setInterval("getruchuNum();", 5000);
    ...
    $("#btnPrint").click(function () {
        ...
        if (rowid == null) {
            ...
            var url = "PrintBasicOutList?Info=" + escape(texts)+"&type=1";
    ...
    $("#Fin").click(function () {
        ...
        if (ids == null) {
            ...
            var ProOutUser = Model.ProOutUser;
            $.ajax({
                type: "POST",
                url: "UpHomeProductSalesState",
                data: { ListOutID: ListOutID, ProOutUser: ProOutUser },
...
function AddRetailSalesOut() {
    window.parent.parent.OpenDialog("新增出库单", "../InventoryManage/AddHomeProductSales", 800, 450);
}
function jq() {
    ...
    jQuery("#list").jqGrid({
        url: 'HomeProductSalesList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, ListOutID: ListOutID, HouseID: HouseID, Begin: Begin, End: End, State: State, OrderContactor: OrderContactor },
        loadonce: false,
        mtype: 'POST',
        jsonReader: {
            root: function (obj) {
        ...
        colNames: ['', '序号', '出库单号', '出库时间', '库管', '经手人', '总金额', '所属仓库',  '备注', '状态','订单状态','订单编号'],
        colModel: [
       ...
       // { name: 'SubjectName', index: 'SubjectName', width: 150, align: "center" }, '科目',
        ...
        { name: 'OrderID', index: 'OrderID', width: 80, align: "center" }
        ],
        pager: jQuery('#pager'),
        pgbuttons: true,
        rowNum: OnePageCount,
...
function jq1(ListOutID) {
    ...
    jQuery("#list1").jqGrid({
        ...
        colNames: ['序号', '产品编号', '产品名称', '规格型号', '单位', '申请数量', '实际数量', '单价', '厂家', '批号', '备注', '订单号'],
        colModel: [
        { name: 'RowNumber', index: 'RowNumber', width: 50, align: "center" },
        ...
        loadComplete: function () {
            $("#list1").jqGrid("setGridHeight", $("#pageContent").height() / 2 - 170, false);
            ...
            if (json.length > 0) {
                for (var i = 0; i < json.length; i++) {
                    ...
                    if (json[0].num != "" && json[0].num != 0) {
                        $("#alarm1").show();
                        warnstring += "<p><a style='cursor:pointer;color:red;' onclick=\"Clickhu('../InventoryManage/HomeProductSales')\">有未签收订单:'" + json[0].num + "'</a></p>";
                    } else {
                        $("#alarm1").hide();
                    }
