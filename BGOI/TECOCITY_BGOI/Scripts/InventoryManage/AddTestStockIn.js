...
$(document).ready(function () {
    ...
    $("#btnSave").click(function () {
        ...
        for (var i = 0; i < tbody.rows.length; i++) {
            ...
            var unitPrice = document.getElementById("ExUnitNo" + i).value;//tbody.getElementsByTagName("tr")[i].cells[6].innerText;//document.getElementById("ExUnitNo" + i).innerHTML;
            var totalAmount = document.getElementById("ExTotalNo" + i).value;
            ...
            var tid = tbody.getElementsByTagName("tr")[i].cells[10].innerText;//document.getElementById("TID" + i).innerHTML;
        ...
        $.ajax({
            ...
            type: "Post",
            data: {
                ListInID: ListInID, SubjectID: SubjectID, BatchID: BatchID, StockInTime: StockInTime, HouseID: HouseID, ProPlace: ProPlace,TID:TID,
...
function addProDetail(Eid, Tid) {
    //var strTID = $("#TID").val();
    //$("#TID").val(strTID + "," + Tid);
    $.ajax({
        url: "GetSalesReturnTask",
        type: "post",
        data: { eid: Eid },
        dataType: "json",
        success: function (data) {
            ...
            if (json.length > 0) {
                for (var i = 0; i < json.length; i++) {
                  ...
                  //  html += '<td ><lable class="labRowNumber' + rowCount + ' " id="RowNumber' + rowCount + '">' + CountRows + '</lable> </td>';
                    ...
                    html += '<td ><input type="text" id="ExUnitNo' + rowCount + '" style="width:30px;" onblur="OnBlurUnitPrice(this);" value="' + json[i].Price2 + '"/></td>';

                   // html += '<td ><lable class="labExUnitNo' + rowCount + ' " id="ExUnitNo' + rowCount + '">' + json[i].ExUnitNo + '</lable> </td>';
                    html += '<td ><input type="text" id="ExTotalNo' + rowCount + '" value="' + json[i].ExTotalNo + '" style="width:60px;"/></td>';
                    //html += '<td ><lable class="labExTotalNo' + rowCount + ' " id="ExTotalNo' + rowCount + '">' + json[i].ExTotalNo + '</lable></td>';
                    ...
                    html += '<td  style="display:none;"><lable class="labTID' + rowCount + ' " id="TID' + rowCount + '">' + Tid + '</lable> </td>';
...
function AddNewTest()
{
    ShowIframe1("选择退货检验单", "../InventoryManage/ChangeTest", 500, 250);
}
function GetAmount() {
    $("#AmountM").val("");
    var Amount = 0;
...
function DelRow() {
    ...
    if (one == false)
        ...
        var rowIndex = -1;
        var typeNames = ["RowNumber", "PID", "OrderContent", "Specifications", "Unit", "Amount", "ExUnitNo", "ExTotalNo", "Supplier", "Remark", "TID", 'Count'];
...
function OnBlurAmount(rowcount) //求金额和
{
    ...
    if (parseInt(AmountC) < parseInt(Count)) {
        ...
        $("#" + newCount).val("0");
        $("#ExTotalNo" + strRow).val("0");
        return;
    }
    
    //var strU = "#ExUnitNo" + strRow;
    //var strUnitPrice = $(strU).text();
    var strUnitPrice = document.getElementById("ExUnitNo" + strRow).value;//
    ...
    if (isNaN(strTotal)) {
        $("#ExTotalNo" + strRow).val("");
    }
    else {
        $("#ExTotalNo" + strRow).val(strTotal);
    }
