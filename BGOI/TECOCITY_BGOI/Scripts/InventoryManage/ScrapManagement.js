   ...
   // jq1("");
    ...
    $("#Fin").click(function () {
        ...
        if (ids == null) {
            ...
            var ListScrapID = Model.ListScrapID;
            $.ajax({
                type: "POST",
                url: "UpScrapDateState",
                data: { ListScrapID: ListScrapID },
...
function ScrapManagementOut() {
    window.parent.parent.OpenDialog("新建报废单", "../InventoryManage/ScrapManagementOut", 800, 450);
}
function jq() {
    ...
    jQuery("#list").jqGrid({
        url: 'ScrapManagementList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, ListScrapID: ListScrapID, HouseID: HouseID, Begin: Begin, End: End, State: State, Handlers: Handlers, PID: PID },
        loadonce: false,
        mtype: 'POST',
        jsonReader: {
            root: function (obj) {
        ...
        colNames: ['','序号', '报废单单编号', '货物编号', '报废时间', '数量', '金额', '经办人', '仓库', '库房类型','报废原因', '状态'],
        colModel: [
            { name: 'IDCheck', index: 'Id', width: 20, hidden: true },
        { name: 'RowNumber', index: 'RowNumber', width: 50, align: "center" },
        { name: 'ListScrapID', index: 'ListScrapID', width: 120, align: "center" },
        { name: 'PID', index: 'PID', width: 150, align: "center" },
        { name: 'ScrapTime', index: 'ScrapTime', width: 100, align: "center" },
        { name: 'ScrapCount', index: 'ScrapCount', width: 80, align: "center" },
        { name: 'AmountM', index: 'AmountM', width: 100, align: "center" },
        { name: 'Handlers', index: 'Handlers', width: 80, align: "center" },
        { name: 'HouseName', index: 'HouseName', width: 100, align: "center" },
        { name: 'Text', index: 'Text', width: 80, align: "center" },
        { name: 'ReasonRemark', index: 'ReasonRemark', width: 80, align: "center" },
...
function reload() {
    ...
    $("#list").jqGrid('setGridParam', {
        url: 'ScrapManagementList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, ListScrapID: ListScrapID, HouseID: HouseID, Begin: Begin, End: End, State: State, PID: PID },
...
function getSearch() {
    ...
    $("#list").jqGrid('setGridParam', {
        url: 'ScrapManagementList',
        datatype: 'json',
        postData: { curpage: curPage, rownum: OnePageCount, ListScrapID: ListScrapID, HouseID: HouseID, Handlers: Handlers, Begin: Begin, End: End, State: State, PID: PID },
        loadonce: false

    }).trigger("reloadGrid");//重新载入
    reload1("");
}
function jq1(ListScrapID) {
    jQuery("#list1").jqGrid({
        url: 'StockOutDetialList',
        datatype: 'json',
        postData: { curpage: curPage1, rownum: OnePageCount1, ListScrapID: ListScrapID },
        loadonce: false,
        mtype: 'POST',
        jsonReader: {
            root: function (obj) {
        ...
        colNames: ['序号', '产品编号', '产品名称', '规格型号', '单位', '申请数量', '实际数量', '单价', '厂家', '备注'],
        colModel: [
        { name: 'RowNumber', index: 'RowNumber', width: 50, align: "center" },
...
function FX() {
    ...
    for (var i = 0; i < rowid.length; i++) {
        var m = jQuery("#list").jqGrid('getRowData', rowid[i]).ListScrapID;
        str += "'" + m + "',";
    }
    $("#ListScrapIDN").val(str);
}