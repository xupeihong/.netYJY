...
$(document).ready(function () {
   ...
   // $("#StockInTime").val("");
    $("#btnSave").click(function () {
        ...
        var ApplyPer = $("#ApplyPer").val();
        var ApplyTime = $("#ApplyTime").val();

        
        if (ApplyPer == "" || ApplyPer == null) {
            alert("申请人不能为空");
            return;
        }
       ...
       // var UnitName = "";
        ...
        for (var i = 0; i < tbody.rows.length; i++) {
           ...
           // var unitName = document.getElementById("Unit" + i).innerHTML;
            ...
            var manufacturer = tbody.getElementsByTagName("tr")[i].cells[7].innerText;//document.getElementById("Supplier" + i).innerHTML;
            var remark = tbody.getElementsByTagName("tr")[i].cells[8].innerText;//document.getElementById("Remark" + i).innerHTML;
            var did = tbody.getElementsByTagName("tr")[i].cells[9].innerText;//document.getElementById("DID" + i).innerHTML;
            var orderId = tbody.getElementsByTagName("tr")[i].cells[10].innerText;//document.getElementById("OrderID" + i).innerHTML;
        ...
        $.ajax({
            url: "SaveProtoStockIn",
            type: "Post",
            data: {
                ListInID: ListInID, SubjectID: SubjectID, BatchID: BatchID, StockInTime: StockInTime, HouseID: HouseID, ProPlace: ProPlace, DID:DID,ApplyPer:ApplyPer,ApplyTime:ApplyTime,
                ...
                StockInCount: StockInCount, UnitPrice: UnitPrice, TotalAmount: TotalAmount, Manufacturer: Manufacturer, Remark: Remark, Amount: Amount
            },
            async: false,
                ...
                for (var i = 0; i < json.length; i++) {
                  ...
                  //  html += '<td ><lable class="labRowNumber' + rowCount + ' " id="RowNumber' + rowCount + '">' + CountRows + '</lable> </td>';
                   ...
                   //html += '<td ><lable class="labUnit' + rowCount + ' " id="Unit' + rowCount + '">' + json[i].Unit + '</lable> </td>';
                    ...
                    html += '<td ><input type="text" id="UnitPrice' + rowCount + '" value="' + json[i].UnitPrice + '" style="width:60px;" onblur="OnBlurUnitPrice(this);" /></td>';
                    ...
                    html += '<td style="display:none;"><lable class="labOrderID' + rowCount + ' " id="OrderID' + rowCount + '">' + json[i].PAID + '</lable> </td>';
...
function AddNewTest() {
    ShowIframe1("选择撤样机货品", "../InventoryManage/ChangeProto", 700, 350);
}
function GetAmount() {
    $("#AmountM").val("");
    var Amount = 0;
...
function OnBlurAmount(rowcount) //求金额和
{
    ...
    var strUnitPrice = document.getElementById("UnitPrice" + strRow).value; //document.getElementById("DetailInfo").getElementsByTagName("tr")[strRow].cells[5].innerText;
