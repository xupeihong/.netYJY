...
$(document).ready(function () {
    ...
    $("#uploadify").uploadify({
        uploader: 'InsertProCompletionsFile',           // 服务器端处理地址
        ...
        formData: { 'PID': $("#StrPID").val(), 'sid': $("#Strcid").val() },// 'Types': $("#StrType").val(), 'Comments': $("#StrComments").val()
        // 两个配套使用
    ...
    $("#charge").click(function () {
        ...
        if ($("#StrCompleteDate").val() == "") {
            alert("竣工日期不能为空");
            return;
        }
        if ($("#StrCompletePerson").val() == "") {
            alert("验收人不能为空");
            return;
        }
        if ($("#StrCompleteRmark").val() == "") {
            alert("验收说明不能为空");
            return;
        }
        isConfirm = confirm("确定要修改竣工验收内容吗")
        if (isConfirm == false) {
            return false;
        }
        else
