...
$(document).ready(function () {
    ...
    $('#SP').click(function () {
        ...
        if (rowid == null) {
            ...
            var PID = jQuery("#list").jqGrid('getRowData', rowid).PayID;
...
function jq() {
    ...
    jQuery("#list").jqGrid({
        ...
        colNames: ['', '项目编号', 'PID', '内部编号', '费用支出编号', '项目名称', '项目负责人', '立项时间', '项目状态', 'State', '审批编号', '所属单位'],
        colModel: [
...
function select(rowid) {
    ...
    RelenvceID = jQuery("#list").jqGrid('getRowData', rowid).PayID;
    reload1();
    reload2();
}
...
function jq2() {
    jQuery("#list2").jqGrid({
        ...
        colNames: ['', '费用支出编号', '操作内容', '操作结果', '操作时间', '操作人'],
        colModel: [
        { name: 'IDCheck', index: 'Id', width: 20, hidden: true },
