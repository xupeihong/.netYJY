...
$(document).ready(function () {
    ...
    $('#XG').click(function () {
        ...
        if (rowid == null) {
            ...
            window.parent.OpenDialog("分包设计修改", "../ProjectManage/UpdateSubPackage?id=" + texts, 700, 570, '');
        }
    })
    ...
    $('#DY').click(function () {
        ...
        var texts = jQuery("#list").jqGrid('getRowData', rowid).SID;
        var url = "PrintSubPackage?id=" + escape(texts);
    ...
    $('#TJSP').click(function () {
        ...
        if (rowid == null) {
            ...
            var texts = jQuery("#list").jqGrid('getRowData', rowid).SID + "@" + "分包设计审批";
...
function jq() {
    ...
    jQuery("#list").jqGrid({
        ...
        colNames: ['', '项目编号', 'PID', '内部编号', '项目名称', '立项时间', '项目负责人', '分包设计编号', '分包设计单位', '分包设计单位项目负责人', '分包设计费用', '分包设计原因', '分包设计主要内容', '所属单位', '状态', 'state'],
        colModel: [
        ...
        { name: 'DesignUnit', index: 'DesignUnit', width: 120 },
        { name: 'DesignPrincipal', index: 'DesignPrincipal', width: 120 },
        { name: 'DesignPrice', index: 'DesignPrice', width: 120 },
        { name: 'SubReason', index: 'SubReason', width: 120 },
