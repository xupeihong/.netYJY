...
$(document).ready(function () {
    ...
    //
    var IsOutUT = $("#StrIsOutUT").val();
    if (IsOutUT == 0) {
        $("input[name=IsOutUT]:eq(0)").attr("checked", 'checked');
    }
    else {
        $("input[name=IsOutUT]:eq(1)").attr("checked", 'checked');
    }
    //
    var FirstCheckUT = $("#StrFirstCheckUT").val();
    if (FirstCheckUT == 0) {
    ...
    var SecondCheckUT = $("#StrSecondCheckUT").val();
    if (SecondCheckUT == 0) {
    ...
    var ThirdCheckUT = $("#StrThirdCheckUT").val();
    if (ThirdCheckUT == 0) {
    ...
    //
    ShowInfoUT($("#StrIsOutUT").val());

    // 确定
    $("#QRXG").click(function () {
        isConfirm = confirm("确定要修改超声波登记卡吗")
        if (isConfirm == false) {
            return false;
        }
        else {
            var tab = document.getElementById("CheckItemsUT");
            ...
            $("#HTitleUT").val(strTitle);
            $("#HCheckedUT").val(strChecked);
