...
function GetDetailInfo() {
    $.ajax({
        url: "GetSendDeliveryInfo",
        type: "post",
        data: { DeliverID: $("#strDeliverID").val() },
        dataType: "json",
        async: false, //是否异步
...
function FillData(data) {
    ...
    if (data.SendDeliveryInfo != "") {
        var tab = $("#DetailInfo");
        tab.html('');
        var datas = eval("(" + data.SendDeliveryInfo + ")");
        $.each(datas.SendDeliveryInfo, function (i, n) {
        ...
        if (datas.SendDeliveryInfo.length < 10) {
            for (var row = 12; row > datas.SendDeliveryInfo.length; row--) {
                var tr = $('<tr style="height:30px;"></tr>');
...
function mc(tableId, startRow, endRow, col) {
    ...
    for (var i = startRow; i < endRow; i++) {
        if (tb.rows[startRow].cells[0].innerHTML == tb.rows[i + 1].cells[col].innerHTML) {
