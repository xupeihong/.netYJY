...
$(document).ready(function () {
    ...
    $('#SP').click(function () {
        ...
        if (rowid == null) {
            ...
            var PID = jQuery("#list").jqGrid('getRowData', rowid).BPID;
    ...
    if ($('.field-validation-error').length == 0) {
        ...
        if (strDateS <= strDateE) {
            ...
            $("#list").jqGrid('setGridParam', {
                ...
                datatype: 'json',
                postData: {
                    curpage: curPage, rownum: OnePageCount, PlanID: PlanID, PlanName: PlanName, MainContent: MainContent,
                    SpecsModels: SpecsModels, BelongArea: BelongArea, State: State, StartDate: StartDate,
...
function jq() {
    ...
    jQuery("#list").jqGrid({
        ...
        postData: {
            curpage: curPage, rownum: OnePageCount, PlanID: PlanID, PlanName: PlanName, MainContent: MainContent,
            SpecsModels: SpecsModels, BelongArea: BelongArea, State: State, StartDate: StartDate, EndDate: EndDate, Op: Op
        },
        loadonce: false,
        mtype: 'POST',
        jsonReader: {
        ...
        colNames: ['', '项目编号', '项目名称', '备案状态', '审批状态', 'SPstate', '审批编号', ''],
        colModel: [
        { name: 'IDCheck', index: 'Id', width: 20 },
        { name: 'BPID', index: 'BPID', width: 150 },
        ...
        { name: 'BPID', index: 'BPID', width: 200, hidden: true }
        ],
        pager: jQuery('#pager'),
        pgbuttons: true,
        rowNum: OnePageCount,
...
function select(rowid) {
    ...
    RelenvceID = jQuery("#list").jqGrid('getRowData', rowid).BPID
    reload1();
    reload2();
}
