       ...
       // var reg2 = /^0\d{2,3}-?\d{7,8}?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
        ...
        { }
            ...
            //if (!!Tel.match(((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$) == false) {
              if(reg.test(Tel)==false){  alert("请重新输入电话号码");
                return;
            }
        }
        ...
        $.ajax({
            ...
            type: "Post",
            data: {
                PID: PID, ExpectedTime: ExpectedTime, PlanID: PlanID, PlanName: PlanName, WorkChief: WorkChief, Tel: Tel,
...
function DelRow() {
    ...
    //var rowid = $("#DetailInfo").jqGrid('getGridParam', 'selrow');
    //if (rowid == null) {
    //    alert("请选择要删除的行");
    //    return false;
    //}
