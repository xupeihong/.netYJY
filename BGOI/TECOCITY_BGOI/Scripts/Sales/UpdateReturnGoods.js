        ...
        success: function (data) {
            ...
            if (json.length > 0) {
                ...
                for (var i = 0; i < json.length; i++) {
                    ...
                    html += '<td ><input type="text" onblur=RXJ(this)  id="RUnitPrice' + rowCount2 + '" style="width:90px;" value="' + json[i].ExUnit + '" /> </td>';
                    html += '<td ><input type="text"  readonly="readonly" id="RSubtotal' + rowCount2 + '"  style="width:90px;" value="' + json[i].ExTotal + '" /></td>';
        ...
        success: function (data) {
            ...
            if (json.length > 0) {
                ...
                for (var i = 0; i < json.length; i++) {
                    ...
                    html += '<td ><input type="text"  class="labSupplier' + rowCount + '"  id="RSupplier' + rowCount + '" value="' + json[i].Manufacturer + '"/></td>';
                    html += '<td ><input  type="text" onblur=RXJ(this) id="RAmount' + rowCount + '" value="' + json[i].ActualAmount + '"/></td>';

                    html += '<td ><input type="text" onblur=RXJ(this)  id="RUnitPrice' + rowCount + '" value="' + json[i].Price + '"/></td>';
                    html += '<td ><input type="text" readonly="readonly" id="RSubtotal' + rowCount + '" value="' + json[i].ActualSubTotal + '"/></td>';
...
function XJ(rowid) {
   ...
   // var strRow = newPrice.charAt(newPrice.length - 1);
    ...
    var strUnitPrice = $("#UnitPrice" + s).val();
    var Count = $("#Amount" + s).val();
...
function RXJ(rowid) {
    ...
    RowId = rowid.id;
    var a = RowId.split('RAmount');
    var b = RowId.split('RUnitPrice');
    var Total = 0;
    var s = "";
    //s = a[1];
    if (a.length == 2) {
 ...
 //   var newPrice = rowPrice.id;
  //  var strRow = newPrice.charAt(newPrice.length - 1);
    var strUnitPrice = $("#RUnitPrice" + s).val();
    var Count = $("#RAmount" + s).val();
       ...
       // var EID = "";
        ...
        $.ajax({
            ...
            data: {
                ...
                RAmount: RAmount, RSupplier: RSupplier, RUnitPrice: RUnitPrice, RSubtotal: RSubtotal, EDID: EDID
