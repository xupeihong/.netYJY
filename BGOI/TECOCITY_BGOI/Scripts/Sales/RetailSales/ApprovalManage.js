...
function reload() {
    if ($('.field-validation-error').length == 0) {
        ...
        $("#list").jqGrid('setGridParam', {
            url: 'SearchApproval',
            datatype: 'json',
            postData: { curpage: curPage, rownum: OnePageCount, IOID: IOID, SalesProduct: SalesProduct, StartDate: StartDate, EndDate: EndDate, Op: Op },
...
function jq(caption) {
    ...
    jQuery("#list").jqGrid({
        ...
        colNames: ['', '申请编号', '产品名称', '创建时间', '任务状态', 'State', '审批编号'],
        colModel: [
        { name: 'IDCheck', index: 'Id', width: 20 },
        { name: 'IOID', index: 'IOID', width: 150 },
        { name: 'OrderContent', index: 'OrderContent', width: $("#bor").width() - 700 },
...
function select(rowid) {
    ...
    RelenvceID = jQuery("#list").jqGrid('getRowData', rowid).IOID;
    reload1();
    reload2();
}
