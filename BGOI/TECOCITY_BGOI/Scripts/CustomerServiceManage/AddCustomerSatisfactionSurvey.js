...
$(document).ready(function () {
    ...
    $("#btnSave").click(function () {
        ...
        for (var i = 0; i < tbody.rows.length; i++) {
            var Productid = tbody.getElementsByTagName("tr")[i].cells[1].innerText;//document.getElementById("PID" + i).innerHTML;
            ...
            var orderForm = document.getElementById("OrderForm" + i).value;
            var orderDate = document.getElementById("OrderDate" + i).value;
        ...
        $.ajax({
            ...
            data: {
                ...
                CreateUser: CreateUser, Type: Type,Other:Other,HFIDID:HFIDID,
                ID: ID, ProductID: ProductID, MainContent: MainContent, SpecsModels: SpecsModels,
                OrderNum: OrderNum, OrderForm: OrderForm, OrderDate: OrderDate
            },
            async: false,
...
function jq() { //增加货品信息行
    ...
    $.ajax({
        url: "GetCustomerSatisfactionSurveyUserVisit",
        type: "post",
        data: { HFID: HFID },
        dataType: "json",
        success: function (data) {
            ...
            if (json.length > 0) {
                for (var i = 0; i < json.length; i++) {
                    ...
                    html += '<td ><lable style="width:150px;" class="labSpecsModels' + rowCount + ' " id="SpecsModels' + rowCount + '">' + json[i].SpecsModels + '</lable> </td>';
                    html += '<td ><input type="text" id="OrderNum' + rowCount + '"  value="" style="width:80px;" /></td>';
                    html += '<td ><input type="text" id="OrderForm' + rowCount + '"  value="" style="width:80px;" /></td>';
                    html += '<td ><input type="text" id="OrderDate' + rowCount + '" onclick="WdatePicker()" class="Wdate" style="width: 170px;" /></td>';
                  //  html += '<td ><input type="text" id="OrderDate' + rowCount + '" onclick="WdatePicker()"   value="" style="width:80px;" /></td>';
                    html += '</tr>'
                    $("#DetailInfo").append(html);
