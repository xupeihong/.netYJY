...
$(document).ready(function () {
    ...
    $('#SP').click(function () {
        ...
        if (rowid == null) {
            ...
            var PID = jQuery("#list").jqGrid('getRowData', rowid).CRID;
...
function reload() {
    if ($('.field-validation-error').length == 0) {
        HandleUser = $('#HandleUser').val();
        $("#list").jqGrid('setGridParam', {
            url: 'UserBillingRecordsProcessing',
            datatype: 'json',
            postData: { curpage: curPage, rownum: OnePageCount, HandleUser: HandleUser },
        }).trigger("reloadGrid");
    }
}
...
function jq() {
    ...
    jQuery("#list").jqGrid({
        ...
        colNames: ['日期', '付款单位', '收款额', '付款内容', '付款方式', '收款人', '公司财务', '备注', '编号', '开票记录编号', '审批状态', '审批编号', '投诉编号'],
        colModel: [
        ...
        { name: 'CRID', index: 'CRID', width: 200, align: "center" }
        ],
        pager: jQuery('#pager'),
        pgbuttons: true,
        rowNum: OnePageCount,
