...
$(document).ready(function () {
    ...
    $("#btnSave").click(function () {
        ...
        var CustomerID = $("#CustomerID option:selected").text();//客户ID
        ...
        $.ajax({
            ...
            data: {
                ...
                HandleDate: HandleDate, CostDate: CostDate, CustomerFeedback: CustomerFeedback,OrderID:OrderID,
                EmergencyDegree: EmergencyDegree, ComplaintCategory: ComplaintCategory,HandleUser:HandleUser,
                FirstDealUser: FirstDealUser, UntiID: UntiID, UserName: UserName, Type: Type,Adderss:Adderss,Tel:Tel,
...
function jq() {
    ...
    $("#Hidden1").val(TSID);
    $.ajax({
        url: "UpModifyUserComplaintsList",
        type: "post",
        data: { TSID: TSID },
        dataType: "json",
        success: function (data) {
            ...
            for (var i = 0; i < json.length; i++) {
                DepNameNew(json[i].UntiID);
                $("#CustomerID").val(json[i].Customer);
                $("#RecordDate").val(json[i].RecordDate);
                $("#ComplaintTheme").val(json[i].ComplaintTheme);
                $("#ComplaintDate").val(json[i].ComplaintDate);

                //  $(".EmergencyDegree").find("option[text='" + json[i].EmergencyDegree + "']").attr("selected", true);


                $("#EmergencyDegree option:contains('" + json[i].EmergencyDegree + "')").prop('selected', true);
                $("#FirstDealUser option:contains('" + json[i].UserName + "')").prop('selected', true);
                $("#UntiID option:contains('" + json[i].UntiID + "')").prop('selected', true);
                $("#UserName option:contains('" + json[i].UserName + "')").prop('selected', true);
                $("#ComplainContent").val(json[i].ComplainContent);
                ...
                $("#Tel").val(json[i].Tel);
                $("#Adderss").val(json[i].Adderss);
            }
        }
    })

    $.ajax({
        url: "UpModifyUserComplaintsListPro",
        type: "post",
        data: { TSID: TSID },
        dataType: "json",
        success: function (data) {
...
function deleteTr(date) {
    ...
    if (one == false)
        ...
        var PID = document.getElementById("DetailInfo").getElementsByTagName("tr")[PIDpro].cells[6].innerText;
        $("#oldpid").val(PID);
...
function CheckDetail() {
    ...
    ShowIframe1("选择产品信息", "../CustomerService/OrderList?OrderContactor=" + OrderContactor, 500, 350);
}
function DelRow() {
    var tbody = document.getElementById("DetailInfo");
    var rowCount = tbody.rows.length;
