...
function unique(arr) {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        if (i == 0) {
            ...
            $("#bor1").append("  <div id='lingjian" + i + "'style='overflow-y: scroll; border: 1px solid #707070;    margin-left: 10px; height: 200px'> <table   cellpadding='0' cellspacing='0' class='tabInfo'>  <tr align='center' valign='middle'>   <th class='th'> 序号 </th><th class='th'> 成品ID </th> <th class='th'> 零件ID </th><th class='th'> 零件名称 </th> <th class='th'> 规格型号 </th><th class='th'> 总数量 </th> <th class='th'> 已收货数量 </th> <th class='th'> 本次收货数量 </th>  <th class='th'> 单位 </th> <th class='th'> 操作 </th>   <tbody id='GXInfosupplier" + i + "'></tbody></table></div>");
        }
        else {
            $("#supplier").append(" <input  id='supplierss" + i + "' class='btnTh' type='button' onclick=Suppliers(this," + result.length + ") value='" + result[i] + "' />");
            $("#bor1").append("  <div id='lingjian" + i + "' style='overflow-y: scroll; border: 1px solid #707070;   display:none; margin-left: 10px; height: 200px'><table   cellpadding='0' cellspacing='0' class='tabInfo'>  <tr align='center' valign='middle'>   <th class='th'> 序号 </th><th class='th'> 成品ID </th><th class='th'> 零件ID </th> <th class='th'> 零件名称 </th> <th class='th'> 规格型号 </th><th class='th'> 总数量 </th> <th class='th'> 已收货数量 </th> <th class='th'> 本次收货数量 </th> <th class='th'> 单位 </th>  <th class='th'> 操作 </th>   <tbody id='GXInfosupplier" + i + "'></tbody></table> </div>");
...
function Suppliers(rowid, lenght) {
            ...
            success: function (data) {
                ...
                if (json.length > 0) {
                    for (var i = 0; i < json.length; i++) {
                        ...
                        html += '<td  style="display:none"><lable class="labProductID' + rowCount + ' " id="DDDID' + rowCount + '">' + json[i].Bak + '</lable> </td>';
                        ...
                        html += '<td ><lable class="labOrderContent' + rowCount + ' " id="LJnums' + rowCount + '">' + json[i].DDAmount + '</lable> </td>';
                        ...
                        html += '<td  style="display:none" ><lable class="labOrderContent' + rowCount + ' " id="LJZQnums' + rowCount + '">' + json[i].Amount + '</lable> </td>';
                        html += '<td ><lable class="labOrderContent' + rowCount + ' " id="LJYSnums' + rowCount + '">' + json[i].DDActualAmount + '</lable> </td>';
                        //html += '<td ><lable class="labOrderContent' + rowCount + ' " id="LJBCnums' + rowCount + '">' + json[i].Number * numberss[1] + '</lable> </td>';
                        html += '<td ><input type="text" id="LJBCnums' + rowCount + '" value="' + json[i].Number * numberss[1] + '"  style="width:150px;"/></td>';
    ...
    htmls += '  <td>收货说明</td>';
    htmls += '<td colspan=8><input type="text" id="ArrivalDescription" style="width:590px;" /></td>';
...
function ADDFK(table) {
    ...
    for (var i = 0; i < tbodys.rows.length - 1; i++) {
        ...
        LJBCSHnums = parseInt(LJYSnums) + (parseInt(LJBCnums) - parseInt(LJZQnums));
    ...
    if (isConfirm == false) {
        ...
        $.ajax({
            ...
            type: "Post",
            data: {
                ljrownumber: LJrownumber, ljid: LJid, ljname: LJname, ljspec: LJspec, ljnums: LJnums, ljmanufacturer: LJmanufacturer, ljunits: LJunits ,ljbcshnums: LJbcshnums, ljbcnums: LJbcnums, dddid: DDdid, ljcpid: LJCPid,
